...
import com.lusen.cardola.BuildConfig;


/**
 * ListView, which is capable to pin section views at its top while the rest is still scrolled.
 * Fork from https://github.com/beworker/pinned-section-listview/
 * Demo: https://github.com/beworker/pinned-section-listview/blob/master/example/src/com/hb/examples/PinnedSectionListActivity.java
 */
public class DetailSectionListView extends OverScrollListView {
    ...
    private final AbsListView.OnScrollListener mOnScrollListener = new AbsListView.OnScrollListener() {
        ...
        public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
            ...
            int bottom = getItemCurrentY(firstVisibleItem);
            if (bottom > mOffsetY) {

            } else {
                firstVisibleItem = (firstVisibleItem + 1) > adapter.getCount() - 1 ? adapter.getCount() - 1 : (firstVisibleItem + 1);
            }
            ...
            if (isFirstVisibleItemSection) {
                ...
                if (sectionView.getTop() == getPaddingTop() + mOffsetY) { // view sticks to the top, no need for pinned shadow
                    destroyPinnedShadow();
    ...
    public int getItemCurrentY(int position) {
        View c = getChildAt(0);
        if (c == null) {
            return 0;
        }
...
//        return getItemCurrentY(firstVisiblePosition);
//    }
    ...
    public DetailSectionListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView();
    }

    public DetailSectionListView(Context context, AttributeSet attrs, int defStyle) {
    ...
    void createPinnedShadow(int position) {
        ...
        View pinnedView = getAdapter().getView(position, pinnedShadow.view, DetailSectionListView.this);
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mPinnedSection != null && mPinnedSection.view != null) {
            ...
            int pTop = getListPaddingTop() + mOffsetY;
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if (mTouchTarget != null) {
            if (isPinnedViewTouched(mTouchTarget, x, y)) { // forward event to pinned view
                MotionEvent evClone = MotionEvent.obtain(ev);
                evClone.offsetLocation(0, -mOffsetY);
                mTouchTarget.dispatchTouchEvent(evClone);
            }
    ...
    private boolean isPinnedViewTouched(View view, float x, float y) {
        ...
        mTouchRect.top += mTranslateY + mOffsetY;

        mTouchRect.bottom += mTranslateY + getPaddingTop() + mOffsetY;
    ...
    private boolean performPinnedItemClick() {
        ...
        try {
            isEnable = getAdapter().isEnabled(mPinnedSection.position);
        } catch (Exception e) {
        }
