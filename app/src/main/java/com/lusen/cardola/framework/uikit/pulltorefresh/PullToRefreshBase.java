...
import com.lusen.cardola.framework.uikit.pulltorefresh.internal.CircleProgressLoadingLayout;
import com.lusen.cardola.framework.uikit.pulltorefresh.internal.FlipLoadingLayout;
import com.lusen.cardola.framework.uikit.pulltorefresh.internal.LoadingLayout;
import com.lusen.cardola.framework.uikit.pulltorefresh.internal.RotateLoadingLayout;
import com.lusen.cardola.framework.uikit.pulltorefresh.internal.Utils;
import com.lusen.cardola.framework.uikit.pulltorefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    private boolean mIsRefreshFailed = false;
    ...
    private OnInterceptPullRefreshListener mOnInterceptPullRefreshListener;
...
//        AppLog.d("2222 dispatchTouchEvent = " + actionMask + "," + actionIndex + "," + pointerId + "," + count);
...
//        int pointerId = event.getPointerId(actionIndex);
//        int count = event.getPointerCount();
//        AppLog.d("2222 onInterceptTouchEvent = " + actionMask + "," + actionIndex + "," + pointerId + "," + count);
...
//        int pointerId = event.getPointerId(actionIndex);
//        int count = event.getPointerCount();
//        AppLog.d("2222 onTouchEvent = " + actionMask + "," + actionIndex + "," + pointerId + "," + count);
    ...
    public final void setOnPullBeforeRefreshListener(OnInterceptPullRefreshListener listener) {
        mOnInterceptPullRefreshListener = listener;
    }
    ...
    public final void setRefreshing(boolean doScroll) {
        if (!isRefreshing()) {
            if (null != mOnInterceptPullRefreshListener && !mOnInterceptPullRefreshListener.isPullBeforeToRefreshEnabled()
                    && mOnInterceptPullRefreshListener.getBeforeRefreshLabelResourceId() != 0) {
                onPullBeforeRefresh(mOnInterceptPullRefreshListener.getBeforeRefreshLabelResourceId());
            }else{
    ...
    //added by eason.syc
    protected void onBeginPull() {
        switch (mCurrentMode) {
            case PULL_FROM_END:
                mFooterLayout.beginPull();
                break;
            case PULL_FROM_START:
                mHeaderLayout.beginPull();
                break;
            default:
                // NO-OP
                break;
    ...
    public void onResetLoadingLayoutHeader(){
        switch (mCurrentMode){
            case PULL_FROM_END:
                mFooterLayout.resetHeader();
                break;
            case PULL_FROM_START:
                mHeaderLayout.resetHeader();
                break;
            default:
                // NO-OP
                break;
    ...
    protected void onPullBeforeRefresh(int label) {
        switch (mCurrentMode) {
            case PULL_FROM_END:
                mFooterLayout.pullBeforeRefresh(label);
                break;
            case PULL_FROM_START:
                mHeaderLayout.pullBeforeRefresh(label);
                break;
            default:
                // NO-OP
                break;
    ...
    protected void onReset() {
        ...
        if (mIsRefreshFailed) {//added by eason.syc
            switch (mCurrentMode) {
                case PULL_FROM_END:
                    mFooterLayout.refreshFailed();
                    break;
                case PULL_FROM_START:
                    mHeaderLayout.refreshFailed();
                    break;
                default:
                    // NO-OP
                    break;
    ...
    private void pullEvent() {
        ...
        if (newScrollValue != 0 && !isRefreshing()) {
            ...
            if (mState != State.PULL_TO_REFRESH && itemDimension >= Math.abs(newScrollValue)) {
                ...
                if (mOnInterceptPullRefreshListener == null || mOnInterceptPullRefreshListener.isPullBeforeToRefreshEnabled()) {
    ...
    private final void smoothScrollTo(int newScrollValue, long duration, long delayMillis,
        ...
        if (oldScrollValue != newScrollValue && !mIsRefreshFailed) {//mIsRefreshFailed add by eason.syc
    ...
    public static enum AnimationStyle {
        ...
        /**
         * use a progress bar depending on where the user has scrolled
         */
        PROGRESS;
        ...
        LoadingLayout createLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs) {
            switch (this) {
                ...
                case PROGRESS:
                    return new CircleProgressLoadingLayout(context,mode,scrollDirection,attrs);
            }
        }
    }
    ...
    public static enum State {
        ...
        /**
         * When the UI is currently refreshing, caused by a call to setRefreshing()}.
         */
    ...
    public void setHasMore(boolean hasMoreData){
        ...
        if (mOnInterceptPullRefreshListener != null){
            mOnInterceptPullRefreshListener.setHasMore(hasMoreData);
        }
        if (hasMoreData){
            onResetLoadingLayoutHeader();
        }
    }
...
//    public void onSwipeUpEvent(SwipeUpEvent swipeUpEvent){
//        onTouchEvent(swipeUpEvent.mEvent);
//    }
